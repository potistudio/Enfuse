...
impl<'a> canvas::Program<WaveformOutput> for Waveform<'a> {
	...
	fn draw(
		...
		let geometry = self.cache.draw(renderer, bounds.size(), |frame| {
			// Background
			frame.fill_rectangle(Point::ORIGIN, bounds.size(), Color::from_rgba(0.0, 0.0, 0.0, 0.2));
			...
			if !self.data.is_empty() {
				...
				let start_index = (current_index as isize - visible_points as isize).max(0) as usize;
				let end_index = (current_index as usize + visible_points).min(self.data.len().saturating_sub(1));

				if start_index < self.data.len() && pixels_per_point > 0.0 {
					let mut prev_x = center_x + (start_index as f32 - current_index) * pixels_per_point;
					let mut prev_y = (height / 2.0) - (self.data[start_index].amplitude * (height / 2.0));
					...
					for i in (start_index + 1)..=end_index {
						...
						let stroke = canvas::Stroke {
							style: canvas::Style::Solid(Color::from_rgb(p.color[0], p.color[1], p.color[2])),
							width: 1.5,
				...
				// Center Line
				frame.fill_rectangle(Point::new(center_x, 0.0), Size::new(2.0, height), CENTER_LINE_COLOR);
				...
				if self.bpm > 0.0 && pixels_per_point > 0.0 {
					...
					let beat_offset_in_points = self.beat_offset * self.sample_rate as f32 / samples_per_point;
					...
					// オフセットを考慮したビート番号の計算
					let start_beat = ((min_index - beat_offset_in_points) / points_per_beat).ceil() as i32;
					let end_beat = ((max_index - beat_offset_in_points) / points_per_beat).floor() as i32;
